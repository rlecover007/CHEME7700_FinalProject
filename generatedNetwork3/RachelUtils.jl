...
function runCaSim()
	...
	idx_small = find(resSS.<0)
	resSS[idx_small]=0.0 #remove small negative concentrations
	IC = setIC_40microM_ADP_Active(data_dictionary, resSS[end,:])
...
function plotNaProfiles()
	...
	dhap_id = findfirst(metabs, "dhap_c")
	close("all")
	...
	figure(figsize=[15,15])
	PyPlot.plot(time_total*60*60, res_total[:,dhap_id], "kD")
	PyPlot.plot(time_total_KO*60*60, res_total_KO[:,dhap_id], color = "r")
	legend(["Wild Type Cytosolic DHAP", "PTGS1 Knocked Out Cytosolic DHAP"], loc = "best")
	xlabel("Time in seconds", fontsize =20)
	ylabel("Concentration in microMolar")
end
